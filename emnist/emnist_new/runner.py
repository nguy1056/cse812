...
for i in range(NUM_SIMS):
    ...
    fl.simulation.start_simulation(
        ...
        strategy=strategy,
        client_manager=FLrceSPRT_client_manager()
    )
    now = datetime.now()
    with open('results/FLrceSPRT_accuracies_alpha1.0_' + now.strftime("%Y%m%d%H%M") + '.txt', 'w') as fp:
        for item in test_acc:
            # write each item on a new line
            fp.write("%f\n" % item)
    with open('results/FLrceSPRT_clients_alpha1.0_' + now.strftime("%Y%m%d%H%M") + '.txt', 'w') as fp:
        for item in selected_clients:
            # write each item on a new line
            fp.write("%s\n" % item)
    with open('results/FLrceSPRT_inference_alpha1.0_' + now.strftime("%Y%m%d%H%M") + '.txt', 'w') as fp:
        for item in Inf:
            # write each item on a new line
            fp.write("%f\n" % item)
